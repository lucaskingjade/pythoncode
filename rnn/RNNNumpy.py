...
class RNNNumpy:
    ...
    def calculate_total_loss(self, x, y):
        ...
        for i in np.arange(len(y)):
            ...
            L += np.mean(np.abs(y[i] - o))  #abs difference
            #print(L)
        return L
    ...
    def bptt(self, x, y):
        ...
        for i in range(T):
            delta_o[i] = np.abs(delta_o[i] - y[i])
    ...
    def bptt2(self, x, y):
        ...
        #print("bptt2 sum error")
        #print(delta_o.sum())
        for i in range(T):
            delta_o[i] = delta_o[i] * (1 - delta_o[i])
...
def train_with_sgd(model, X_train, y_train, learning_rate=0.00005, nepoch=100, evaluate_loss_after=5):
    ...
    for epoch in range(nepoch):
        ...
        for i in range(len(y_train)):
            ...
            model.gradient_check(X_train[i], y_train[i])
            loss = model.calculate_loss(X_train, y_train)
            print("loss")
            print(loss)
            num_examples_seen += 1
